<template>
  <div class="flex flex-col">
    <template v-for="(l, i) in links" :key="i">
      <NuxtLink v-if="!l.items" :to="l.link" v-slot="{ isActive }" activeClass="bg-[#068ef1] text-white rounded-md"
        class="font-sans inline-flex items-center gap-4 p-3 px-4 text-left text-muted-foreground text-[15px]">
        <Icon v-if="l.icon" :name="l.icon" :class="[isActive ? 'h-5 w-5 text-white' : 'h-5 w-5 text-slate-600']" />
      ...
      <HDisclosure v-slot="{ open }" v-else>
        <HDisclosureButton class="inline-flex items-center justify-between p-3 px-4 text-left text-[15px]">
          <div class="flex items-center gap-4">
            <Icon v-if="l.icon" :name="l.icon" class="h-5 w-5 text-slate-600" />
            <p class="truncate">
              {{ l.title }}
            </p>
          </div>
